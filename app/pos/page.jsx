...
export default function POS() {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      toast.error("Error al buscar categoria");
    } finally {
  ...
  const total = Math.trunc(subtotal - discount + tax);
  ...
  async function handleSave(data) {
    ...
    try {
      ...
      if (!saleResponse.data.data?.id) {
        throw new Error("Error al concretar.");
      }
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen relative bg-background text-foreground">
      ...
      >
        ...
        {cart.length === 0 && (
          <p className="text-muted-foreground">Aún no se han agregado productos</p>
        )}
        ...
        {cart.length > 0 && (
          <div className="mt-6">
            ...
            <Button className="w-full mt-4 text-white" onClick={handleSave}>
              {saving ? "Guardando..." : "Checkout"}
            </Button>
          </div>
        )}
      </div>
